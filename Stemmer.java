...
public class Stemmer {
	...
	InvertedIndex vsm = new InvertedIndex();
	...
	public String stemmerMain(String args, int docId) {
		char[] w = new char[501];
		...
		for (int x = 0; x < charArray.length; x++) {
			...
			if (Character.isLetter((char) ch)) {
				int j = 0;
				while (true && x < charArray.length - 1) {
					...
					x++;
					ch = charArray[x];
					if (!Character.isLetter((char) ch) || x == charArray.length - 1) {
						/* to test add(char ch) */
						for (int c = 0; c < j; c++)
							s.add(w[c]);
